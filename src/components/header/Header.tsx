...
export default function Header() {
  ...
  return (
    <>
      ...
      <header
        id="header"
        className={`tf-header ${isSticky ? "header-sticky" : ""}`}
        style={{ top: isSticky ? "0" : "-100px" }}
      >
        ...
        <ul className="main-nav_menu">
          {navItems.map((item, idx) => (
            <li key={idx} className="menu-item">
              <Link
                href={`#${item.link}`}
                className="tf-btn style-transparent text-body-3 animate-btn scroll-link"
              >
                {item.name}
              </Link>
            </li>
    ...
    <>
      <div className="header-right">
        <div className="btn_group">
          <Link
            href="/"
            className="tf-btn text-body-3 animate-btn d-none d-sm-flex"
          >
            Log in
          </Link>
          {/* <Link
          ...
          <button
            type="button"
            className="btn-menu_mobile d-lg-none"
            onClick={onOpenMenu}
          >
          ...
          <ul className="nav-ul-mb">
            {navItems.map((item, idx) => (
              <li className="nav-mb-item" onClick={onClose} key={idx}>
                <Link
                  href={`#${item.link}`}
                  className="mb-item-link scroll-link"
                >
                  {item.name}
                </Link>
              </li>
